...
class ParentChildRetrieval:
    ...
    def resolve_parent_content(
        ...
        if not column or column == '':
            logger.warning("[Parent-Child 检索] ID %s 是 child 类型但 Column 为空，返回原始内容", current_id)
            return current_content, metadata
        ...
        if column in self._parent_cache:
            ...
            logger.debug("[Parent-Child 检索] 从缓存获取 parent，Column: %s", column)
        else:
        ...
        if parent_data:
            ...
            logger.debug("[Parent-Child 检索] 找到 parent (ID: %s, Column: %s)，使用 parent 内容替换 child 内容", parent_id, column)
            ...
            logger.warning("[Parent-Child 检索] 未找到 Column %s 对应的 parent，返回原始 child 内容", column)
            return current_content, metadata
    ...
    def _find_parent_by_column(self, column: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 确保 column 不为空
            if not column or column == '':
                logger.warning("[Parent-Child 检索] Column 为空，无法查询 parent")
                return None
            ...
            if results and len(results) > 0:
                parent = results[0]
                logger.debug("[Parent-Child 检索] 查询到 parent: ID=%s, Column=%s", parent.get("id"), column)
                return parent
            else:
                logger.warning("[Parent-Child 检索] 未找到 Column %s 的 parent", column)
                return None
                
        except Exception as e:
            logger.error("[Parent-Child 检索] 查询 parent 失败 (Column=%s): %s", column, e)
            import traceback
    ...
    def resolve_multiple_items(
        ...
        for content, metadata, distance in items:
            ...
            if chunk_method == 'parent_child_split' and pc_type == 'child':
                ...
                # 但为了安全，也直接返回
                logger.warning(
                    "[Parent-Child 召回] 未预期的类型: chunk_method=%s, pc_type=%s，直接返回",
                    chunk_method,
                    pc_type,
                )
                resolved_items.append((content, metadata, distance))
