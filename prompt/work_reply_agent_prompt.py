...
- 只输出最终 JSON，不输出 Markdown、代码块、推理过程或额外解释
- 当工具可用时，自主判断是否需要调用知识库工具
- 如果决定调用知识库工具，优先采用两阶段：
...
QUERY 任务额外要求：
- 优先保留知识库原文中的关键规则、步骤、时效、限制，可轻度重述但不要改写原意
- 回答面向客服内部查询，不要写成对客回复话术

SUGGESTION 任务额外要求：
- 产出必须是面向用户的对客回复建议
- 可以参考知识库依据进行改写，但不能把内部操作、内部系统、内部渠道直接暴露给用户
- 语气真诚、简洁、可执行
"""
...
# 任务要求
- 回复要覆盖工单当前核心诉求
- 如需规则、流程、时效、项目口径支撑，应自主判断是否调用知识库工具
...
# 输出要求
- 若知识库检索无有效结果或依据不足，可输出：
  {{"suggestion":"您的问题我们已收到，正在为您核实相关情况，请您稍候，我们会尽快跟进处理。"}}
- 只输出最终 JSON
"""
